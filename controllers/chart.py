    ...
    if request.method == 'GET':
        ...
        for booking in AllBookingObjs:
            ...
            if roomType == 'standard':
                ...
                ListOf_numOfStandardBookingsByMonthDict = [{key: value} for key, value in numOfStandardBookingsByMonthDict.items()]
                
            # repeat step 2 and 3 for deluxe room type
            elif roomType == 'deluxe':
                checkInMonthYear = booking.checkInDate.strftime("%b %Y")
                numOfDeluxeBookingsByMonthDict[checkInMonthYear] += 1
                ListOf_numOfDeluxeBookingsByMonthDict = [{key: value} for key, value in numOfDeluxeBookingsByMonthDict.items()]
        ...
        xyData['deluxe'] = sorted(ListOf_numOfDeluxeBookingsByMonthDict, key=lambda x: datetime.strptime(list(x.keys())[0], "%b %Y"))
        xyData['standard'] = sorted(ListOf_numOfStandardBookingsByMonthDict, key=lambda x: datetime.strptime(list(x.keys())[0], "%b %Y"))
