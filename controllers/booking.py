...
def bookRoom():
    ...
    if request.method == "GET":
        ...
        if dates_validation:
            ...
            updated_user = User.updateGuest(email=current_user.email, guestObj=Guest(passport=passport, country=country))
...
def upload():
    ...
    if request.method == 'POST':      
        ...
        for num, item in enumerate(list(dict_reader)):
            ...
            room_Set = roomSet.createRoom(roomTypeName=roomTypeName, bedTypeName=bedTypeName, amenityItemCodesList=amenityItemCodesList)
            bookingObj = Booking.createBooking(checkInDate=checkInDate, checkOutDate=checkOutDate, roomSet=room_Set, user=existing_user)
            NumOfUploads = num + 1
        upload_status = True
